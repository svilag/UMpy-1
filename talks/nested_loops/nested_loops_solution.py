...
def main():
    ...
    for article in articles:
        for person in article['byline']['person']:
            ...
            # match name:
            #     case [str(_), str(_), None]:
            #         key = f"{name[0]}_{name[1]}"
            #     case [str(_), str(_), str(_)]:
            #         key = '_'.join(name)

            match name:
                case [str(last_name), str(first_name), None]:
                    key = f"{last_name}_{first_name}"
                case [str(last_name), str(first_name), str(middle_name)]:
                    key = '_'.join((last_name, first_name, middle_name))
